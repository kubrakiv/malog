...
def getCustomerManagers(request):
    customer_managers = CustomerManager.objects.filter(client=request.user.client)
    serializer = CustomerManagerSerializer(customer_managers, many=True, context={'request': request})
...
def createCustomerManager(request):
    try:
        ...
        customer = Customer.objects.filter(id=customer_id, client=request.user.client).first() if customer_id else None
        ...
        serializer = CustomerManagerSerializer(customer_manager, many=False, context={'request': request})
...
def deleteCustomerManager(request, pk):
    try:
        customer_manager = CustomerManager.objects.get(id=pk, client=request.user.client)
...
def updateCustomerManager(request, pk):
    ...
    customer_managers = CustomerManager.objects.filter(id=pk, customer_id=customer_id, client=request.user.client)
    ...
    serializer = CustomerManagerSerializer(instance=customer_manager, data=request.data, partial=True, context={'request': request})
